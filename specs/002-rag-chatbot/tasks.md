...
- [ ] T002 Create `backend/requirements.txt` with dependencies: `fastapi==0.109.0`, `uvicorn[standard]==0.24.0`, `openai==1.10.0`, `openai-agents==0.1.0`, `qdrant-client==1.7.0`, `psycopg2-binary==2.9.9`, `asyncpg==0.29.0`, `sqlalchemy==2.0.25`, `langchain-text-splitters==0.0.1`, `python-dotenv==1.0.0`, `pydantic==2.5.3`, `tenacity==8.2.3`, `pytest==7.4.4`, `pytest-asyncio==0.23.3`, `httpx==0.26.0`
...
- [ ] T020 [US1] Create `backend/src/services/agent.py`: Implement RAG agent using OpenAI Agents SDK (https://openai.github.io/openai-agents-python/) with async pattern: `from agents import Agent, Runner, function_tool`; define `@function_tool async def search_textbook(query: str) -> str` that embeds query, searches Qdrant, returns formatted context with sources; create `agent = Agent(name="TextbookAssistant", instructions="Answer ONLY from textbook content. Cite sources. Refuse non-textbook questions.", tools=[search_textbook], model="gpt-4o-mini")`; implement `async def run_rag_agent(question: str, selected_text: str = None) -> dict` that calls `result = await Runner.run(agent, question)` and returns `{answer: result.final_output, sources: extracted_citations, confidence: score}`; if selected_text provided, prepend to question context
